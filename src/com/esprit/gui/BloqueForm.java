 ...
 * @author tayssir
 */
public class BloqueForm extends Form {
    Form f;
    public BloqueForm(Resources theme) {
        super(new BorderLayout(BorderLayout.CENTER_BEHAVIOR_CENTER_ABSOLUTE));
        setUIID("LoginForm");
       
        Container welcome = FlowLayout.encloseCenter(
                new Label("Votre compte est bloqué", "WelcomeBlue")
              
        );
        Button logout = new Button("Log Out");
        logout.setUIID("LoginButton");
        logout.addActionListener(e -> {
           LoginForm ef = new LoginForm(theme);
                ef.show();
                    });
...
/*        TextField login = new TextField("", "Nom", 20, TextField.EMAILADDR) ;
        ...
        loginButton.addActionListener(e -> {
            ...
            bienvenue(login.getText(),password.getText());
            if (Session.isCnx()) {
                System.out.println("Enter YES LOGIN");
                EditForm ef = new EditForm(theme);
                ef.show();
...
//                deboupicLabel,
        ...
        by.setScrollVisible(false);
          ...
          private List<String> bienvenue (String u, String p){
